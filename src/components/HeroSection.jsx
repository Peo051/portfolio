...
const chipItems = ["Full-Stack", ".NET", "SQL", "Clean Architecture", "AI / ML"];
        ...
        <div>
          <AxisReveal axis="y" distance={16} amount={0.5}>
            <p className="inline-flex items-center gap-2 rounded-full border border-sky-300/25 bg-sky-400/10 px-4 py-2 font-mono text-xs uppercase tracking-[0.16em] text-sky-200">
              {textByLang(personalInfo.heroEyebrow, language)}
            </p>
          </AxisReveal>
          ...
          <AxisReveal axis="y" distance={22} delay={0.04} amount={0.45}>
            <h1 className="mt-6 max-w-3xl text-balance text-4xl font-semibold leading-tight text-slate-100 sm:text-5xl lg:text-6xl">
              {textByLang(personalInfo.headline, language)}
            </h1>
          </AxisReveal>

          <AxisReveal axis="y" distance={22} delay={0.08} amount={0.45}>
            <p className="mt-6 max-w-2xl text-base leading-relaxed text-slate-300 sm:text-lg">{textByLang(personalInfo.shortPositioning, language)}</p>
          </AxisReveal>

          <AxisReveal axis="y" distance={16} delay={0.12} amount={0.45}>
            <div className="mt-8 flex flex-wrap gap-3">
              <a href="#projects" className="btn-primary focus-ring">
                {textByLang(uiText.hero.viewProjects, language)} <ArrowUpRight size={16} />
              </a>
              <a href="#contact" className="btn-secondary focus-ring">
                {textByLang(uiText.hero.contactMe, language)}
              </a>
            </div>
          </AxisReveal>
            ...
            <div className="mt-6 flex flex-wrap items-center gap-3 text-sm text-slate-300">
              ...
              {socialLinks.map((link) => {
                ...
                return (
                  ...
                  >
                    <Icon size={14} />
                    {textByLang(link.label, language)}
                  </a>
                );
              })}
            </div>
          ...
          <div className="surface-card p-5 sm:p-6">
            <div className="relative mx-auto max-w-sm">
              <div className="profile-frame">
                <img
                  src={profilePhoto}
                  alt={textByLang(uiText.hero.profileAlt, language)}
            ...
            <div className="mt-5 rounded-2xl border border-line bg-slate-900/70 p-4">
              <p className="font-mono text-[11px] uppercase tracking-[0.14em] text-sky-300/80">{textByLang(uiText.hero.snapshotTitle, language)}</p>
